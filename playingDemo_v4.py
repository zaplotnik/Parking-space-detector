...
relative_threshold = 2000 # 200 pixel for distance 0.1, 24 pixel for distance 0.05
...
while (cap.isOpened()):
    ...
    if iFrame % detection_interval == 0:
        # initialize a tracker on the first frameq
        (h, w) = frame.shape[:2]
        
        ## !! RESIZE TAKES A LOT OF TIME - IMPORTANT TO PIPE LOW-RES VIDEO, e.g. 640x480
        blob = cv2.dnn.blobFromImage(cv2.resize(frame, (426, 300)), 0.007843, (426, 300), 127.5)
